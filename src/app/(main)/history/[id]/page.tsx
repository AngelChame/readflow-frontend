...
export default function HistorySessionPage() {
  ...
  useEffect(() => {
    ...
    const fetchT48Status = async () => {
      ...
      try {
        ...
        const quizInfo =
          data.quiz ?? (data.quizData ? { available: true } : null);
  ...
  return (
    <div className="w-full h-full flex flex-col">
      <div className="h-fit flex flex-row space-x-2">
        {menuTypes.map((menuType, index) => (
          <button
            ...
            className={`px-3 py-2 sm:px-6 sm:py-3 text-sm sm:text-base font-medium transition-colors duration-300 rounded-t-2xl ${
              active === index
      ...
      <div className="flex-1 min-h-0 p-4 sm:p-8 md:p-16 bg-background-secondary rounded-b-2xl rounded-r-2xl flex flex-col relative">
        {/* resumen */}
        {active === 0 && (
            ...
            {session.attempts.t0 &&
              ...
              t48Status !== null && (
                <HistoryQuizModal
                  ...
                  minutesRemaining={
                    !t48Status.available
                      ? t48Status.minutesRemaining
                      : undefined
                  }
                  buttonText={
                    t48Status.available ? "Realizar quiz espaciado" : undefined
                  }
                  onAction={
                    t48Status.available
                      ? () => router.push(`/session/${id}/test`)
                      : undefined
                  }
                />
              )}
