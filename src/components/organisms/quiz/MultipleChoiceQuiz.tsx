...
export default function MultipleChoiceQuiz({ data }: Props) {
                ...
                <div className="space-y-2">
                    {feedback.detail.map((item, i) => (
                        <div key={i} className={`rounded-xl border p-4 ${item.isCorrect ? 'border-green-500/30 bg-green-500/10' : 'border-red-500/30 bg-red-500/10'}`}>
                            ...
                            {!item.isCorrect && (<p className="text-xs text-muted-foreground mt-2 pl-4">{item.explanation}</p>)}
                        </div>
                    ))}
                </div>
    ...
    // Vista de quiz
    return (
        <div className="w-full h-full bg-background flex flex-col items-center align-middle justify-center">
            <div className="w-full h-full bg-background-secondary rounded-2xl flex-col items-center align-middle py-20 px-40 shadow-sm space-y-6">
                ...
                <p className="text-xl font-semibold text-foreground leading-snug">{q.question}</p>
                ...
                <div className="space-y-5">
                    {q.options.map((option, i) => (
                        <button
                            ...
                            className={`w-full text-left rounded-xl px-5 py-6 text-sm font-medium transition-all duration-300 border-2 hover:cursor-pointer
                            ${selected === i
