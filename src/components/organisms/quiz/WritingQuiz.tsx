...
export default function WritingQuiz({ data }: Props) {
  ...
  if (finished && result) {
    ...
    return (
      <div className="w-full h-full bg-background-secondary rounded-2xl flex flex-col p-4 shadow-sm space-y-4 overflow-y-auto pb-24 md:pb-4">
        <div>
          <h1 className="text-1xl font-bold text-foreground">
            {passed ? "¡Bien hecho!" : "Sigue practicando"}
          </h1>
          <p className="text-muted-foreground mt-1">
            Identificaste {feedback.concepts_found.length} de{" "}
            {criteria.required_concepts.length} conceptos clave
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="rounded-xl border border-border bg-background p-4 text-center">
            <p className="text-xl font-bold text-green-500">
              {feedback.concepts_found.length}
            </p>
          ...
          <div className="rounded-xl border border-border bg-background p-4 text-center">
            <p className="text-xl font-bold text-red-400">
              {feedback.concepts_missing.length}
            </p>
            <p className="text-sm text-muted-foreground mt-1">
              Conceptos faltantes
            </p>
  ...
  // Vista de quiz
  return (
    <div className="w-full h-full bg-background-secondary overflow-y-auto rounded-2xl px-4 sm:px-16 md:px-40 flex items-center justify-center">
      <div className="w-full space-y-6 py-6 md:pb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            ...
            Redacta un texto de lo aprendido
          </p>
        </div>
        ...
        <p
          className={`text-xs text-right ${canSubmit ? "text-muted-foreground" : "text-muted-foreground/40"}`}
        >
          {text.trim().length} caracteres{" "}
          {!canSubmit && `(mínimo ${MIN_CHARS})`}
        </p>
